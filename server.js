...
// ── DATABASE — auto-create directory so Railway volume works ──────
const DB_PATH = process.env.DATABASE_PATH || './razco.db';
...
function normalizePhone(raw) { return (raw || '').replace(/\D/g, '').slice(-10); }
function twilioPhone(tenDigit) { return '+1' + tenDigit; }
...
app.get('/health', (req, res) => {
  ...
  res.json({ status:'ok', service:'Razi-Nova Backend', customers:n, db_path:path.resolve(DB_PATH), twilio:!!twilioClient, openai:!!openai });
});
...
app.post('/api/customers/signup', (req, res) => {
  ...
  activePCs.forEach(pc => { db.prepare('INSERT INTO customer_punches (customer_id, punch_card_id, punches) VALUES (?,?,0)').run(customer.id, pc.id); });
...
app.get('/api/punch-cards', (req, res) => {
  res.json({ success:true, punch_cards:db.prepare('SELECT * FROM punch_cards ORDER BY created_at DESC').all() });
});
app.post('/api/punch-cards', (req, res) => {
  ...
  db.prepare('SELECT id FROM customers').all().forEach(c => { db.prepare('INSERT OR IGNORE INTO customer_punches (customer_id,punch_card_id,punches) VALUES (?,?,0)').run(c.id, card.id); });
...
app.delete('/api/punch-cards/:id', (req, res) => { db.prepare('DELETE FROM punch_cards WHERE id=?').run(req.params.id); res.json({ success:true }); });

app.get('/api/coupons', (req, res) => { res.json({ success:true, coupons:db.prepare('SELECT * FROM coupons ORDER BY created_at DESC').all() }); });
...
app.delete('/api/coupons/:id', (req, res) => { db.prepare('DELETE FROM coupons WHERE id=?').run(req.params.id); res.json({ success:true }); });

app.get('/api/campaigns', (req, res) => { res.json({ success:true, campaigns:db.prepare('SELECT * FROM campaigns ORDER BY created_at DESC').all() }); });
...
app.post('/api/campaigns/:id/send', async (req, res) => {
  ...
  let sent=0, failed=0;
  for (const c of customers) { const r = await sendSMS(twilioPhone(c.phone), campaign.message); if (r.success) sent++; else failed++; }
...
app.post('/api/campaigns/send-now', async (req, res) => {
  ...
  for (const c of customers) { const r = await sendSMS(twilioPhone(c.phone), message); if (r.success) sent++; else failed++; }
...
app.post('/api/flash', async (req, res) => {
  ...
  const customers = getOptedInCustomers(target||'all');
  let sent=0;
  for (const c of customers) { const r = await sendSMS(twilioPhone(c.phone), message); if (r.success) sent++; }
  res.json({ success:true, sent, total:customers.length });
});

app.get('/api/raffles', (req, res) => { res.json({ success:true, raffles:db.prepare('SELECT * FROM raffles ORDER BY created_at DESC').all() }); });
...
app.post('/api/raffles/:id/send', async (req, res) => {
  ...
  for (const c of customers) {
    ...
    const r = await sendSMS(twilioPhone(c.phone), msg); if (r.success) sent++;
  }
  res.json({ success:true, sent, total:customers.length });
});
...
app.get('/api/weekly-ad', (req, res) => {
  ...
  try { ad.images = JSON.parse(ad.images || '[]'); } catch(e) { ad.images = []; }
  res.json({ success:true, ad });
});
app.post('/api/weekly-ad', (req, res) => {
  const { dates, images } = req.body;
  if (!images || !Array.isArray(images)) return res.status(400).json({ success:false, error:'images array required' });
...
app.post('/api/ai/write', async (req, res) => {
  ...
  try {
    const prompt = `Write a short bilingual (English + Spanish) SMS marketing message for Razco Foods Supermarket in Lindsay, CA. Type: ${type}. Audience: ${audience||'all customers'}. ${notes||''}. Keep under 160 characters. Warm, family-oriented tone. End with "Reply STOP to unsubscribe". Return only the message text.`;
...
app.post('/api/ai/shopping-list', async (req, res) => {
  ...
  try {
    const prompt = `Write a personalized shopping list SMS for ${customer.name}, a Razco Foods customer with ${customer.visits} visits and ${customer.points} points (${customer.tier} tier). Current week: ${ad?.dates||'this week'}. Bilingual (English+Spanish), under 300 chars, 3-4 items. End with "¡Te esperamos! 💚"`;
    const r = await openai.chat.completions.create({ model:'gpt-3.5-turbo', messages:[{role:'user',content:prompt}], max_tokens:200 });
    res.json({ success:true, message:r.choices[0].message.content.trim() });
  } catch(e) { res.json({ success:false, error:e.message }); }
});
...
app.post('/api/settings', (req, res) => {
  Object.entries(req.body).forEach(([k,v]) => { db.prepare('INSERT OR REPLACE INTO settings (key,value) VALUES (?,?)').run(k, v); });
...
cron.schedule('0 8 * * 1', async () => {
  ...
  if (!ad) return;
  const customers = db.prepare("SELECT * FROM customers WHERE opt_in_sms=1").all();
  for (const c of customers) { await sendSMS(twilioPhone(c.phone), `🛒 ¡Esta semana en Razco Foods! ${ad.dates}. ¡Ven y ahorra familia! 💚 Reply STOP`); }
});
